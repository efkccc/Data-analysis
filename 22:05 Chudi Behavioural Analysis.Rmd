...
######################################
conf_prob_det<- conf_prob %>%
  filter(task=="Detection") %>%
  group_by(task, stimulus, response,accuracy, confidence) %>%
  summarise(mean_prob=mean(prob))
conf_prob_det

roc(conf_prob_det$accuracy, conf_prob_det$mean_prob, plot=TRUE, legacy.axes=TRUE,
    percent = TRUE, xlab="False positive rate", ylab="Hit rate", main="Detection Type 1 ROC", col="#1f78b4", lwd=4, print.auc=TRUE)

####################################
#ROC discrimination plotting
####################################
conf_prob_dis<- conf_prob %>%
  filter(task=="Discrimination") %>%
  group_by(task, stimulus, response,accuracy, confidence) %>%
  summarise(mean_prob=mean(prob))
conf_prob_dis

roc(conf_prob_dis$accuracy, conf_prob_dis$mean_prob, plot=TRUE, legacy.axes=TRUE,
    percent = TRUE, xlab="False positive rate", ylab="Hit rate", main="Discrimination Type 1 ROC", col="#fc8d62", lwd=4, print.auc=TRUE)

#####################################
#ROC tilt plotting
#####################################
conf_prob_til<- conf_prob %>%
  filter(task=="Tilt") %>%
  group_by(task, stimulus, response,accuracy, confidence) %>%
  summarise(mean_prob=mean(prob))
conf_prob_til

roc(conf_prob_til$accuracy, conf_prob_til$mean_prob, plot=TRUE, legacy.axes=TRUE,
    percent = TRUE, xlab="False positive rate", ylab="Hit rate", main="Tilt Type 1 ROC", col="#8dd3c7",lwd=4, print.auc=TRUE)
```
&nbsp;
