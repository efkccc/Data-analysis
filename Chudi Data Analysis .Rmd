...
ggplot(data, mapping = aes(x= task, y=correct)) + ylim(0.4, 1) +labs(title="Mean accuracy across three tasks", y="mean accuracy")+ theme_classic() +
...
# MM: You have three different variables names hit_detect, det_hit, and hit_det :)
# this is confusing and error-prone. We want to give our variables meaningful names 
# that will be understandable by others, but more importantly - but our future selves :)
...
# MM: I think these were a useful exercise in the way to getting the beautiful type-2 ROC curves,
# but not sure they are strictly necessary at this stage. I leave it to you to decide. If you do keep them, 
# (as you can probably guess :)), I would write a function instead of writing the same code three times.
...
# 4.Type 2 ROC curves
```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, fig.width=3.5, fig.height=3.5, cache=TRUE}

# MM: Same here, better to write a function :)
...
BOLD_det_suj <- aggregate(Data_Det[, 16:21], list(Data_Det$subj_id, Data_Det$confidence, Data_Det$response ), mean)
#rename the colum names 


#MM: what is the meaning of these column names (Group.1 etc?)
BOLD_det_suj <- rename(BOLD_det_suj,c('subj_id'='Group.1', 'confidence'='Group.2', 'response'='Group.3'))
#get the group mean of activation for each ROI
BOLD_det_mean <- aggregate(BOLD_det_suj[, 4:9], list(BOLD_det_suj$confidence, BOLD_det_suj$response ), mean)
#get the se of activation for each ROI
BOLD_det_se <- aggregate(BOLD_det_suj[, 4:9], list(BOLD_det_suj$confidence, BOLD_det_suj$response ), se)
...
#get the mean activation at each ROI for each participants at each confidence level for each response
BOLD_dis_suj <- aggregate(Data_Dis[, 16:21], list(Data_Dis$subj_id, Data_Dis$confidence, Data_Dis$response ), mean)
#rename the colum names 
BOLD_dis_suj <- rename(BOLD_dis_suj,c('subj_id'='Group.1', 'confidence'='Group.2', 'response'='Group.3'))
#get the group mean of activation for each ROI
BOLD_dis_mean <- aggregate(BOLD_dis_suj[, 4:9], list(BOLD_dis_suj$confidence, BOLD_dis_suj$response ), mean)
#get the se of activation for each ROI
BOLD_dis_se <- aggregate(BOLD_dis_suj[, 4:9], list(BOLD_dis_suj$confidence, BOLD_dis_suj$response ), se)
#combine with mean df==> ".x==mean, .y==se"
...
#get the mean activation at each ROI for each participants at each confidence level for each response
BOLD_til_suj <- aggregate(Data_Til[, 16:21], list(Data_Til$subj_id, Data_Til$confidence, Data_Til$response ), mean)
#rename the colum names 
BOLD_til_suj <- rename(BOLD_til_suj,c('subj_id'='Group.1', 'confidence'='Group.2', 'response'='Group.3'))
#get the group mean of activation for each ROI
BOLD_til_mean <- aggregate(BOLD_til_suj[, 4:9], list(BOLD_til_suj$confidence, BOLD_til_suj$response ), mean)
#get the se of activation for each ROI
BOLD_til_se <- aggregate(BOLD_til_suj[, 4:9], list(BOLD_til_suj$confidence, BOLD_til_suj$response ), se)
#combine with mean df==> ".x==mean, .y==se"
...
# MM: This plotting should be in a function
...
```{r mixed_effects, eval=FALSE, echo=FALSE, cache=TRUE}
...
plot_re_coefficients <- function(cur_task, ROI, df) {
  ...
  plot <-ggplot(data=coefs, aes(x=confidence, y=estimate)) +
    ...
    labs(title=paste(ROI,cur_task), x= "confidence", y="mean activation", color="Response") +
    theme_classic() + scale_x_continuous(limits = c(0.5, 6),breaks=number_ticks(6)) + scale_y_continuous(limits = c(-1, 1), breaks=number_ticks(6))+
    theme(legend.position=c(0.7, 0.15),  
                   legend.background = element_blank(),
                   legend.key = element_blank(),
