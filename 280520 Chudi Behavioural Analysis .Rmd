...
Behavioural_subj <- Behavioural_Valid %>% 
  group_by(subj_id) %>%
  summarise(totaltrials=(count=n()))
Behavioural_subj
no_subj <- nrow(Behavioural_subj)

Behavioural_Det <- Behavioural_Valid %>% 
  #only include detection 
  filter(task=="Detection")
Behavioural_Det

Behavioural_Dis <- Behavioural_Valid %>% 
  #only include detection 
  filter(task=="Discrimination")
Behavioural_Dis

Behavioural_Til<- Behavioural_Valid %>% 
  #only include detection 
  filter(task=="Tilt")
Behavioural_Til
...
ggplot(data, mapping = aes(x= task, y=correct)) + ylim(0.4, 1) +labs(title="Fig.1 Mean accuracy", y="mean accuracy")+ theme_classic() +
...
tdet <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE, data=Behavioural_Det)$statistic
p.det <- t.test(log_rt~response, mu=0, alt="two.sided",conf=0.95,var.equal=TRUE, data=Behavioural_Det)$p.value
tdis <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE,data=Behavioural_Dis)$statistic
p.dis <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE,data=Behavioural_Dis)$p.value
ttil <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE,data=Behavioural_Til)$statistic
p.til <-t.test(log_rt~response, mu=0, alt="two.sided",conf=0.95,var.equal=TRUE,data=Behavioural_Til)$p.value
...
confidence_distribution <- Behavioural_Valid %>%
...
```{r include=FALSE}
det <- Behavioural_Valid %>%
  filter(task=="Detection") 
det

dis <- Behavioural_Valid %>%
  filter(task=="Discrimination") 
dis

til <- Behavioural_Valid %>%
  filter(task=="Tilt") 
til
...
acc <- Behavioural_Valid %>%
  group_by(subj_id, task, accuracy, response) %>%
  summarise(total_acc_trials=(count=n())) 
acc

conf_prob_acc <- Behavioural_Valid %>%
...
confdet0 <- rbind(confdet0_0,confdet0_1)
...
confdet1 <- rbind(confdet1_0,confdet1_1)
...
confdis0 <- rbind(confdis0_0,confdis0_1)
...
confdis1 <- rbind(confdis1_0,confdis1_1)
...
conftil0 <- rbind(conftil0_0,conftil0_1)
...
conftil1 <- rbind(conftil1_0,conftil1_1)
