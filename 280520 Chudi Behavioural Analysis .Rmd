...
The probability of responding YES in detection was `r format(round(print(det_bias), 2), nsmall = 2)` (± `r format(round(print(sd_bias1), 2), nsmall = 2)`), and was significantly different from 0.5 (ADD T TEST RESULTS). The probability of responding CLOCKWISE was `r format(round(print(dis_bias), 2), nsmall = 2)` (± `r format(round(print(sd_bias2), 2), nsmall = 2)`) and was not significantly different from 0.5. For the tilt recognition task, the probability of responding TILTED was (`r format(round(print(til_bias), 2), nsmall = 2)` ± `r format(round(print(sd_bias3), 2), nsmall = 2)`).

Response time was faster for correct response (1st quartile= `r format(round(print(qt1_correct), 2), nsmall = 2)`, median= `r format(round(unname(getmedian['correct']), 2), nsmall = 2)`, 3rd quartile= `r format(round(print(qt3_correct), 2), nsmall = 2)` milliseconds) than incorrect responses (1st quartile= `r format(round(print(qt1_incorrect), 2), nsmall = 2)`, median= `r format(round(unname(getmedian['incorrect']), 2), nsmall = 2)`, 3rd quartile= `r format(round(print(qt3_incorrect), 2), nsmall = 2)` milliseconds). A one-way analysis of variance failed to detect a significant overall effect of responses type in detection (YES vs. NO, t=`r format(round(print(tdet), 2), nsmall = 2)` p=`r format(round(print(p.det), 2), nsmall = 2)` ), discrimination (CLOCKWISE vs. ANTICLOCKWISE, t=`r format(round(print(tdis), 2), nsmall = 2)`, p= `r format(round(print(p.dis), 2), nsmall = 2)`) and tilt recognition (VERTICAL vs. TILTED, t=`r format(round(print(ttil), 2), nsmall = 2)`, p= `r format(round(print(p.til), 2), nsmall = 2)`). 
...
auc1 <- auc(confdet_acc$prob0, confdet_acc$prob1, from=0, rule=2, yleft=0)
...
Metacognitive sensitivity, which is quantified as the area under Type 2 ROC curve, is significantly higher for Yes (`r print(auc[[4]])`) than No (`r print(auc[[3]])`) response.

```{r echo=FALSE, results = FALSE, message=FALSE, warning=FALSE, fig.width=3.5, fig.height=3.5}
########################################
# Neural data preparation detection v1
########################################
Data_Det$response[Data_Det$response=="0"] <- "No"
Data_Det$response[Data_Det$response=="1"] <- "Yes"

BOLD_det<- subset(Data_Det, select=c(confidence, response, BOLD_rTPJ, BOLD_pMFC, BOLD_FPm, BOLD_FPl, BOLD_preSMA, BOLD_vmPFC))
BOLD_det <- BOLD_det[order(BOLD_det[,1,2]),]

BOLD_det1 <- BOLD_det  %>%
  filter(! BOLD_rTPJ=="NaN") %>%
  group_by(confidence, response) %>%
  summarise_each(funs(sum)) %>%
  mutate(rTPJ=BOLD_rTPJ/no_subj) %>%
  mutate(pMFC=BOLD_pMFC/no_subj) %>%
  mutate(FPm=BOLD_FPm/no_subj) %>%
  mutate(FPl=BOLD_FPl/no_subj) %>%
  mutate(preSMA=BOLD_preSMA/no_subj) %>%
  mutate(vmPFC=BOLD_vmPFC/no_subj) 
BOLD_det1

##############################################  
# Neural data preparation discrimination v1
##############################################
Data_Dis$response[Data_Dis$response=="0"] <- "Anticlockwise"
Data_Dis$response[Data_Dis$response=="1"] <- "Clockwise"

BOLD_dis<- subset(Data_Dis, select=c(confidence, response, BOLD_rTPJ, BOLD_pMFC, BOLD_FPm, BOLD_FPl, BOLD_preSMA, BOLD_vmPFC))
BOLD_dis <- BOLD_dis[order(BOLD_dis[,1,2]),]
...
#################################### 
# Neural data preparation tilt v1
####################################
Data_Til$response[Data_Til$response=="0"] <- "Vertical"
Data_Til$response[Data_Til$response=="1"] <- "Tilted"

BOLD_til<- subset(Data_Til, select=c(confidence, response, BOLD_rTPJ, BOLD_pMFC, BOLD_FPm, BOLD_FPl, BOLD_preSMA, BOLD_vmPFC))
BOLD_til <- BOLD_til[order(BOLD_til[,1,2]),]
...
#################################### 
rTPJ_det <- ggplot(data=BOLD_det1, aes(x=confidence, y=rTPJ)) +
  geom_line(aes(colour = factor(response))) +
  geom_point(aes(color = factor(response)), size = 3) + 
  scale_color_manual(values=c("#e41a1c", "#377eb8")) +
  labs(title="rTPJ detection", x= "confidence", y="mean activation", color="Response") +
  theme_classic() + scale_x_continuous(limits = c(0.5, 6),breaks=number_ticks(6)) + scale_y_continuous(limits = c(-3, 3), breaks=number_ticks(6))+
...
rTPJ_dis <- ggplot(data=BOLD_dis, aes(x=confidence, y=rTPJ)) +
  geom_line(aes(colour = factor(response))) +
  geom_point(aes(color = factor(response)), size = 3) + 
  scale_color_manual(values=c("#984ea3", "#4daf4a")) +
...
rTPJ_til <- ggplot(data=BOLD_til, aes(x=confidence, y=rTPJ)) +
  geom_line(aes(colour = factor(response))) +
  geom_point(aes(color = factor(response)), size = 3) + 
  scale_color_manual(values=c("#ff7f00","#ffff33")) +
...
BOLD_det2 <- aggregate(Data_Det[, 16:21], list(Data_Det$confidence, Data_Det$response ), mean)
BOLD_det2 <- rename(BOLD_det2,c('confidence'='Group.1','response'='Group.2'))
...
# Plotting rTPJ detection
#################################### 
rTPJ_det <- ggplot(data=BOLD_det2, aes(x=confidence, y=BOLD_rTPJ)) +
  geom_line(aes(colour = factor(response))) +
  geom_point(aes(color = factor(response)), size = 3) + 
  scale_color_manual(values=c("#e41a1c", "#377eb8")) +
