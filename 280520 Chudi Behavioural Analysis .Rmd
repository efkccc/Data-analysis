...
Behavioural_Valid <- Data %>% 
  ...
  filter(! response=="NaN") 
Behavioural_Valid 
Behavioural_Valid[,'log_rt']=log(Behavioural_Valid[,'response_time'])
...
ggplot(data, mapping = aes(x= task, y=correct)) + ylim(0.4, 1) +labs(title="Fig.1 Mean accuracy", y="mean accuracy")+ theme_classic() +
   geom_boxplot()+ geom_jitter(position=position_jitter(width=.1, height=0), alpha=0.3, size=4) + scale_y_continuous(breaks=number_ticks(12))
...
rt <- Behavioural_Valid %>%
  ...
  summarise(mean(log_rt)) %>%
  rename("mean_rt"="mean(log_rt)")
rt

#calculating mean rt for correct and incorrect trials
rt_accuracy <- rt %>%
  group_by(accuracy) %>%
  summarise(median(mean_rt)) %>%
  rename("median_rt"="median(mean_rt)") 
rt_accuracy
...
p.det <- t.test(log_rt~response, mu=0, alt="two.sided",conf=0.95,data=Behavioural_Det)$p.value
p.dis <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,data=Behavioural_Dis)$p.value
ttil <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,data=Behavioural_Til)$statistic
p.til <-t.test(log_rt~response, data=Behavioural_Til)$p.value
...
conf_dis_sti0 <- inner_join(confdet0,confdet1, by= c("ID"))
...
# Plotting type 1 ROC
####################################
roc1_det <- ggplot(data=conf_dis_sti0, aes(x=prob0, y=prob1)) +
  geom_line() +
...
#Detection
conf_det0 <- subset(conf_dis, accuracy=="0", select=c(confidence, response, prob_acc))
...
conftil_acc$response[conftil_acc$response=="0"] <- "Anticlockwise"
conftil_acc$response[conftil_acc$response=="1"] <- "Clockwise"
...
roc2_det <- ggplot(data=confdet_acc, aes(x=prob0, y=prob1)) +
  ...
  labs(title="Fig.8 Type 2 ROC Detection", x= "p(con|correct)", y="p(con|incorrect)", color="Response")+ 
  theme_classic()+
...
roc2_dis <- ggplot(data=confdis_acc, aes(x=prob0, y=prob1)) +
  ...
  labs(title="Fig.9 Type 2 ROC Discrimination", x= "p(con|correct)", y="p(con|incorrect)", color="Response")+ 
  theme_classic() + 
  theme(legend.position=c(0.9, 0.2),  
                 legend.background = element_blank(),
                 legend.key = element_blank()) 
...
roc2_til <- ggplot(data=conftil_acc, aes(x=prob0, y=prob1)) +
  ...
  labs(title="Fig.10 Type 2 ROC Tilt", x= "p(con|correct)", y="p(con|incorrect)", color="Response")+ 
  theme_classic() + 
  theme(legend.position=c(0.9, 0.2),  
                 legend.background = element_blank(),
                 legend.key = element_blank())
