...
bias_lm <- apa_print(lm(data = subj_bias, Detection~Tilt))
...
conf_dt <- data.table(conf)
cum_prob_subj <- conf_dt[, .(cum_prob=cumsum(prob), prob_se=(se(prob)),confidence), by=.(subj_id, task, response, accuracy)]

sub_02 <- conf %>% filter(subj_id=="02DiSt")
test <- by(sub_02, list(sub_02$accuracy, sub_02$response), function(x){
  unique(sub_02$confidence)
})


test <- by(conf, list(conf$subj_id, conf$accuracy, conf$response), function(x){
  unique(conf$confidence)
})
...
data1 <- test %>% filter(task=="Discrimination")
rt_model <- lm(data = data1, acc~median_rt)
