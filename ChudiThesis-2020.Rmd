...
# create a function to get the mean response and compare with 0.5
...
resp_det <- resp_bias('Yes', response)
...
#calculating rt
#####################
rt <- data_valid %>% group_by(subj_id, task, response, accuracy) %>% summarise(median(rt)) %>%rename("median_rt"="median(rt)")
# transfrom into data.table
dt <- data.table(data_valid)

# get individual median rt
subj_rt <- dt[, .(median_rt=median(rt)), by=.(subj_id,task, response, accuracy)]

# get group median rt
rt <- subj_rt[, .(median_rt=median(median_rt)), by=.(task, response, accuracy)]

# get quantiles of rt
qt_accuracy <- rt[, .(quantile(median_rt, c(.25,.5,.75))), by=accuracy]

#t test comparing log_rt for detection, discrimination and tilt 
t_rt_det <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE, data=data_det)
t_rt_dis <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE,data=data_dis)
t_rt_til <-t.test(log_rt~response, mu=0, alt="two.sided", conf=0.95,var.equal=TRUE,data=data_til)

```

Detection (accuracy= `r apa(task_accuracy$Detection, 2, T)`, d'= `r apa(d_task$mean_d[d_task$response=='Yes'], 2, T)`),  discrimination (accuracy= `r apa(task_accuracy$Discrimination, 2, T)`, d'= `r apa(d_task$mean_d[d_task$response=='Clockwise'], 2, T)`) and tilt recognition (accuracy = `r apa(task_accuracy$Tilt, 2, T)`, d'= `r apa(d_task$mean_d[d_task$response=='Tilted'], 2, T)`) was similar. A one-way ANOVA failed to detect a significant difference between the accuracy of these three tasks `r apa_print(aov_accuracy)` and d' `r apa_print(aov.d)` ; see Figure \ref{fig:accuracy})).

The probability of responding Yes in detection was `r apa(resp_det$mean, 2, T)` (± `r apa(resp_det$sd, 2, T)`), and was significantly different from 0.5 (`r resp_det$apa_t`). The probability of responding CLOCKWISE was `r apa(resp_dis$mean, 2, T)` (± `r apa(resp_dis$sd, 2, T)`) and was not significantly different from 0.5 (`r resp_dis$apa_t`). For the tilt recognition task, the probability of responding TILTED was `r apa(resp_til$mean, 2, T)` ± `r apa(resp_til$sd, 2, T)`, which was not significantly different from responding VERTICAL (`r resp_til$apa_t`).

Response time was faster for correct response (1st quantile= `r qt_accuracy[1,2]`, median= `r qt_accuracy[2,2]`, 3rd quantile= `r qt_accuracy[3,2]` milliseconds) than incorrect responses (1st quantile= `r qt_accuracy[4,2]`, median= `r qt_accuracy[5,2]`, 3rd quantile= `r qt_accuracy[6,2]` milliseconds). A one-way analysis of variance failed to detect a significant overall effect of responses type in detection (Yes vs. NO, `r apa_print(t_rt_det)`) discrimination (CLOCKWISE vs. ANTICLOCKWISE, `r apa_print(t_rt_dis)`) and tilt recognition (VERTICAL vs. TILTED, `r apa_print(t_rt_til)`) on response time.
...
```{r, log_rt_figure, echo=FALSE, message=FALSE,fig.width=3.5, fig.height=3.2, cache=TRUE}
...
# calculating area under the curve for each response
AUC <- as.data.frame(sapply(split(cum_prob, cum_prob$response), function(x) {
  data.table(auc=auc(x=x$cum_prob_0, y=x$cum_prob_1, from=0))
}))
...
cum_prob$response[cum_prob$response=="Tilted"] <- paste0('Vertical (AUC=',apa(AUC$Tilted.auc, 2, T), ')')
cum_prob$response[cum_prob$response=="Yes"] <- paste0('Vertical (AUC=',apa(AUC$Yes.auc, 2, T), ')')
cum_prob$response[cum_prob$response=="No"] <- paste0('Vertical (AUC=',apa(AUC$No.auc, 2, T), ')')
cum_prob$response[cum_prob$response=="Clockwise"] <- paste0('Vertical (AUC=',apa(AUC$Clockwise.auc, 2, T), ')')
cum_prob$response[cum_prob$response=="Anticlockwise"] <- paste0('Vertical (AUC=',apa(AUC$Vertical.auc, 2, T), ')')

```

## 4.4.Metacognitive sensitivity
Metacognitive sensitivity, which is quantified as the area under Type 2 ROC curve (see \ref{fig:type2 ROC} ), is significantly higher for Yes (`r apa(AUC$Yes.auc, 2, T) `) than No (`r apa(AUC$No.auc, 2, T)`) response. Similar pattern is observed in the tilt recognition task, where the AUC is significantly higher for Tilted (`r apa(AUC$Tilted.auc, 2, T)`) than Vertical (`r apa(AUC$Vertical.auc, 2, T)`) responses where as AUC is similar between the two responses in discrimination task. This suggest that participants confidence ratings are more diagnostic to accuracy in the judgments of a target stimulus being present than being absent. Similarly, the correct judgments of a stimulus being tilted is better reflected by participants' confidence ratings than the correct judgments of a stimulus being vertical. 
...
# scatter plot
#################
rt_test <- dt[, .(median_rt=median(rt)), by=.(subj_id,task, stimulus)]%>%spread(key = 'stimulus', value = 'median_rt') %>%rename(present="1", absent="0") %>% mutate(median_rt=present-absent) %>%subset(select=c(subj_id, task, median_rt))

#count no. of correct trials for each task for each participant
test <- data_valid %>% 
  #group by ID, type of task and accuracy
  group_by(subj_id, task, accuracy, stimulus) %>% 
  #count number of correct/incorrect/NAH trials 
  summarize(no_trials=(count = n())) %>%
  #combine with the total data frame
  inner_join(sti_trials, by = c ("subj_id","task")) %>%
  mutate(correct = no_trials / sti_trials) %>%
  filter(accuracy==1) %>%
  subset(select=c(subj_id, task, stimulus, correct)) %>%
  spread(key = 'stimulus', value = 'correct') %>%
  rename(present="1", absent="0") %>%
  mutate(acc=present-absent) %>%
  subset(select=c(subj_id, task, acc)) %>%
  inner_join(rt_test, by = c ("subj_id", "task"))

test_plot <- function(cur_task, df){
  df<- df%>%filter(task==cur_task)
  plot <- ggplot(data=df, aes(x=median_rt, y=acc)) + geom_point()+ labs(title=paste('RT and Accuracy',cur_task), x= "rt(present-absent)", y="accuracy(present-absent)")
  print(plot)
}
