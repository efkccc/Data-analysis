...
\begin{figure}[h]
\begin{center}
\includegraphics[width=!,totalheight=!,scale=0.9]{UCL.png}
\end{center}
\end{figure}
{\setstretch{1.7} 
\par
...
![Classic Type 2 measure and SDI, taken from @kanai2010cc. Classic measures of Type II performance take confidence ratings for correct and incorrect trials and treat high confidence in correct decisons as hit and high confidence in incorrect decisons as false alarm, vice versa. The SDI measure combine confidence ratings for correct rejection and misses, i.e. when participants gave a no response, and high confidence in correct rejection was trated as hit and high confidence for miss was considered as false alarm.](Kanai_Walsh_SDI.png) 
...
![ROC analysis of SDI for each condition. Greater SDI impies more accurate judgements of performance by participants, i.e. better Type II performance. SDI was above chance level in three attentional manipulations (D-F), reflected by the ROC curve situating above diagnal line whereas it is around chance level in three sensory manipulations (A-C). This graph shows that participants metacognitive sensitivity towards absence responses was impaired in sensory manipulations and this impairment was not present in attentional manipulations. Adapted from Kanai and Walsh (2010)](Kanai_Walsh_SDI_performance.png)
...
\par
SDT provides estimates for two important parameters in the process of making a decision in psychophysics, perceptual sensitivity (between signal and noise) and the strategy adopted by the observer. Perceptual sensitivity is visualised as the differences between the means of the noise and signal distributions (d’ in figure 4). The strategy, also known as response criteria (λ), is the boundary for which the observer decides to give the two responses. For example, in figure 4, given evidence x exceeds the value of λ, the observer will give Yes response and for evidence below the value of λ, they would give No response. SDT model allows convenient mathematical transformations to derive sensitivity and criterion directly from behavioural data (Wickens 2002). In a simple psychophysics experiment, there are four possible outcomes of participants responses which is shown in figure 5 below. Provided with the H and FA rate, perceptual sensitivity (d’) can be calculated as z(H) − z(FA) and λ can be calculated as –((z(H)+z(FA))/2). 

![Four possible outcomes of participants' responses.](response_outcomes.png)
\par
SDT not only explains how observers distinguishes signal from noise, it can also be adapted to account for the confidence rating process (Wickens, 2002). As stated before, the construction of the SDT model requires repeating the experiment a lot of times to collect enough data for the calculation of parameters, which posts heavy burden on the observer. An advanced application of SDT lightens such burden on observer by incorporating their confidence ratings towards decisions they made. This provides extra information from a single trial, and instead of a simple Yes or No answer, participants responses now would include a collection of, for example, Sure Yes (Y2), Unsure Yes(Y1), Sure No (N2), Unsure No (N1) in a 2-scale confidence rating experiment. This extra information is useful for inferring the probability of one response being correct; a Y2 rating is more likely to be from a signal trial than is a Y1 rating. These different confidence levels are separated by multiple criteria (see Fig.6), in contrast to one criterion in the elementary framework. In this adapted version of SDT model, the amount of evidence supporting one choice is regarded as relational to the confidence towards that choice and this is visualised as the distance of the sensory sample from the criterion on the distribution plot.
...
![Representation of perceptual tasks from the unequal variance SDT models (a., b. and c. correspond to detection, discrimination and tilt recognition tasks respectively). The upper panel shows two alternative choices in each task. Middle panel represents the possible stimulus in each task and the distributions of sensory evidence. Note here that in detection no sensory evidence is available for absence (No) conditions thus only distribution for presence (Yes) condition is shown. The bottom panel demonstrates the transformation into log likelihood ratio (LLR) based on SDT for each task, where equal variance distributions for two alternative choices implies a linear function of LLR (b.) and unequal variance distributions implied a quadratic function(a, c).](task_structure.png)
...
Based on finding from previous studies, apart from whole brain exploratory analysis, imaging data were analysed with focuses on the following regions of interests as specified in pre-registration:

  * Frontopolar cortices, Medial frontopolar cortex (FPm) and Lateral frontopolar cortex (FPl): These clusters were defined by a connectivity-based parcellation and the right hemisphere mask was used as mirror to create a bilateral mask.
  
  * Posterior medial frontal cortex (pMFC): BA46? 
  
  * Ventromedial prefrontal cortex (vmPFC): The vmPFC was defined as a 8-mm globule around MNI coordinates [0,46,-7] (Neubert 2014) and aligned to the cortical midline.
  
  * Right temporoparietal junction (rTPJ): peak voxel [54,-46, 26]
  
  * Pre-supplementary moror area (preSMA): peak voxel [0,35,47]
...
\newpage
```{r echo=FALSE, message=FALSE, fig.cap="\\label{fig:ROC} Type 2 ROC curve for each task", fig.width=3, fig.height=3}
...
```{r, echo=FALSE, message=FALSE, fig.cap="\\label{fig:log_rt} Log transformed reaction time across three tasks", cache=TRUE}
...
\par
```{r , echo=FALSE, message=FALSE, fig.cap="\\label{fig:rTPJ} Linear effects of confidence on BOLD signals in the right Temporoparietal Junction (rTPJ). Error bars represent the standard error of the mean.", cache=TRUE}
plot_ROI('rTPJ', BOLD_group)
```
\par
```{r , echo=FALSE, message=FALSE, fig.cap="\\label{fig:FPl} Linear effects of confidence on BOLD signals in the Lateral Frontopolar Cortex (FPl). Error bars represent the standard error of the mean.", cache=TRUE}
plot_ROI('FPl', BOLD_group)
```
To investigate whether linear effects of confidence on BOLD signal were influenced by the type of task (Detection vs. Discrimination vs. Tilt recognition) and/or response (Yes vs. No; Clockwise vs. AntiClockwise; Vertical vs. Tilted), linear models with interactions were tested. The effects of confidence failed to show a significant difference between three tasks in all ROIs (all p values > 0.32), according to uncorrected statistical tests for multiple comparisons. However, when looking at the interaction of response within each task, giving Yes or No responses had significant impact on the linear relationship of confidence on BOLD signals in the FPl region ` r apa(FPl_det, 2, T)` and so did giving Clockwise or AntiClockwise (p= `r apa(beta_resp$p_response[beta_resp$task=='Discrimination'& beta_resp$ROI=='FPl'], 3, T)`)(see Figure \ref{fig:FPl}). In the rTPJ, the effect of response type was also significant (p= `r apa(beta_resp$p_response[beta_resp$task=='Discrimination'& beta_resp$ROI=='rTPJ'], 3, T)`) in discrimination (see Figure \ref{fig:rTPJ}). No significant effect between different responses was observed for three tasks in other regions (p values > 0.13).

### 4.2.2. Whole brain analysis
We conducted an uncorrected whole brain analysis based on the Quadratic-Confidence Design Matrix (QC-DM) specified in the preregistration (INSERT LINK HERE)
