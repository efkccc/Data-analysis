...
# 2.Introduction 
As human beings we frequently make statements such as ‘I am confident that…’, reflecting the importance of confidence in daily life. Confidence can be defined as ‘a belief about the validity of our own thought, knowledge or performance that relies on a subjective feeling.’ [@grimaldi2015nbr], which has wide applications in various fields including medical diagnosis, financial choices and eyewitness testimony etc. As a type of subjective judgement, the level of confidence towards one decision can effectuate other subsequent decisions either for oneself or others, such as when an investor decides to drop a stock investment after purchasing. Given its importance, the mechanism supporting confidence formation has been investigated extensively in both animals and humans, leading to developments of behavioural, neural and computational models. 
...
_Miyamoto and colleagues (2018) reported a similar finding in a short-term metamemory task, where non-human primate subjects wagered on the accuracy of their answers while constrained inside an MRI scanner. Similar to our results, BOLD signal in frontopolar area 10 predicted fluctuations in metacognitive sensitivity specifically for 'non-experienced' responses. Furthermore,  reversible silencing of BA10 by means of a GABA-A silencer injection significantly impaired metacognitive judgments for 'no' responses without having an effect on their type-I performance, supporting a causal role of BA10 for high-level representation of absence. Our results thus reveal a striking resemblance in brain activation for metacognitive judments between human and non-human p-rimates at the highest level of the prefrontal hierarchy. Apart from the importance of these results for the study of human metacognition, we believe it should also inform future discussions about the ethical status of these experiments in non-human primates._ 
...
```
## 4.2.Response Bias
To test whether participants were biased towards certain response, we computed the probability of giving each response in three tasks. The probability of responding NO in detection was `r apa(resp_no$mean, 2, T)` (± `r apa(resp_no$sd, 2, T)`), and was  significantly different from 0.5 (`r resp_no$full_result`), showing that participants are more likely to give NO responses than YES response in detection task. The probability of responding CLOCKWISE was `r apa(resp_dis$mean, 2, T)` (± `r apa(resp_dis$sd, 2, T)`) and was not significantly different from 0.5 (`r resp_dis$full_result`). For the tilt recognition task, the probability of responding TILTED was `r apa(resp_til$mean, 2, T)` (± `r apa(resp_til$sd, 2, T)`), which was significantly lower than responding VERTICAL `r apa(resp_ver$mean, 2, T)` ± (`r apa(resp_ver$sd, 2, T)`) (`r resp_til$full_result`).

Furthermore,given the existence of significant response bias in both detection and tilt recognition task, statistical test was conducted to assess whether individuals who are more likely to give NO responses in detection are also more likely to give VERTICAL response in tilt recognition. A linear model was fitted to the probabilities of giving these two response within each individual and this model fit result suggest that this relationship was not significant (`r bias_lm`). 
...
## 4.3.Response time
Consistent with previous studies, response time was faster for correct response (1st quantile= `r qt_accuracy[1,2]`, median= `r qt_accuracy[2,2]`, 3rd quantile= `r qt_accuracy[3,2]` milliseconds) than incorrect responses (1st quantile= `r qt_accuracy[4,2]`, median= `r qt_accuracy[5,2]`, 3rd quantile= `r qt_accuracy[6,2]` milliseconds) according to a paired t test on the log reaction time(`r (apa_print(t_rt_acc)$full_result)`). A one-way analysis of variance failed to detect a significant overall effect of responses type on log reaction time in detection task (YES vs. NO, `r apa_print(t_rt_det)`), discrimination task (CLOCKWISE vs. ANTICLOCKWISE, `r apa_print(t_rt_dis)`) and tilt recognition task `r (apa_print(t_rt_til)$full_result)`) according to the results from paired sample t tests. 
...
rt_accuracy_plot <- ggplot(data=rt_acc, aes(x=median_rt, y=acc)) + 
    ...
    geom_smooth(method=lm, se=FALSE, linetype="dashed",
             color="darkred")+
    theme_classic()+
    labs(title= "Reaction time and accuracy", x= "mean reaction time difference between responses per subject", y="mean accuracy difference between responses per subject")+
  facet_wrap(~task)
...
Within detection, no significant difference in mean confidence was observed between Yes (target present) and NO (target absent) responses (see Fig.4 above) (`r apa_print(t_conf_det)`) and similarly, in the discrimination task participants' mean confidence ratings did not differ significantly between the CLOCKWISE and ANTICLOCKWISE responses (`r apa_print(t_conf_det)`). However the tilt recognition task, a paired sample t test revealed that the effect of response type had a significant impact on mean confidence levels (`r apa_print(t_conf_til)`, such that participants are more confident in TILTED (` r apa(mean_conf_group$confidence[mean_conf_group$response=="Tilted"], 2, T)`) than VERTICAL response (` r apa(mean_conf_group$confidence[mean_conf_group$response=="Vertical"], 2, T)`) (see Figure \ref{fig:confidence}). 
