...
```{r accuracy& d'& rt, include=FALSE, cache=TRUE}
...
Response time was faster for correct response (1st quartile= `r apa(qt_accuracy[1,1], 2, T)`, median= `r apa(qt_accuracy[2,2], 2, T)`, 3rd quartile= `r apa(qt_accuracy[3,2], 2, T)` milliseconds) than incorrect responses (1st quartile= `r apa(qt_accuracy[4,2], 2, T)`, median= `r apa(qt_accuracy[5,2], 2, T)`, 3rd quartile= `r apa(qqt_accuracy[6,2]t, 2, T)` milliseconds). A one-way analysis of variance failed to detect a significant overall effect of responses type in detection (Yes vs. NO, `r apa_print(t_rt_det)`) discrimination (CLOCKWISE vs. ANTICLOCKWISE, `r apa_print(t_rt_dis)`) and tilt recognition (VERTICAL vs. TILTED, `r apa_print(t_rt_til)`) on response time.
